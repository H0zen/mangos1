...
{
    /**
     * @brief Resolves a spell id straight from the DBC store.
     *
     * Only used on the fallback path below, for the window before
     * SpellCatalog::Build() has run. Once the catalog is built nothing here
     * derives anything -- it all becomes a table read.
     */
    SpellEntry const* ResolveFromStore(void const* /*ctx*/, uint32 spellId)
    {
...
{
    SpellInfo const& info = sSpellCatalog.Get(spellId);
    if (info.dbc)
    {
        return info.passive;
    }
    ...
    SpellEntry const* spellInfo = sSpellStore.LookupEntry(spellId);
    return spellInfo && DeriveIsPassiveSpell(spellInfo);
}

/**
 * @brief Determines whether a spell entry is passive.
 *
 * @param spellInfo The spell entry.
 * @return true if the spell is passive; otherwise false.
 */
...
{
    return DeriveIsPassiveSpell(spellInfo);
}
...
SpellSpecific GetSpellSpecific(uint32 spellId)
{
    ...
    return DeriveSpellSpecific(spellInfo, sSpellMgr.GetSpellElixirMask(spellId));
}
...
{
    return DeriveIsPositiveTarget(targetA, targetB);
}

/**
 * @brief Checks whether an explicit target mode is positive.
 *
 * @param targetA The target mode.
 * @return true if the target mode is explicitly positive; otherwise false.
 */
...
bool IsPositiveEffect(SpellEntry const* spellproto, SpellEffectIndex effIndex)
{
    ...
    {
        return info.IsEffectPositive(effIndex);
    }

    return DeriveIsPositiveEffect(spellproto, effIndex, &ResolveFromStore, NULL);
}
