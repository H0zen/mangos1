...
PathFinder::PathFinder(const Unit* owner, uint32 mapId) :
    ...
    m_budgetStop(Nav::RouteStop::Reached),
    m_sourceUnit(owner), m_navMesh(NULL), m_navMeshQuery(NULL)
{
    ...
    m_profile = Nav::ProfileOf(*m_sourceUnit);
    applyFilter();
}
                           ...
                           Nav::SearchBudget budget)
{
    ...
    m_route.stop = Nav::RouteStop::Failed;
    m_budgetStop = Nav::RouteStop::Reached;
...
void PathFinder::noteSearchLimit(dtStatus status, const char* where)
{
    ...
    {
        if (m_budgetStop == Nav::RouteStop::Reached)
        {
            m_budgetStop = Nav::RouteStop::NodeBudget;
        }
        DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING,
    ...
    if (dtStatusDetail(status, DT_BUFFER_TOO_SMALL))
    {
        if (m_budgetStop == Nav::RouteStop::Reached)
        {
            m_budgetStop = Nav::RouteStop::PolyBudget;
        }
        DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING,
                         "++ PathFinder::%s :: %u filled the polygon buffer (%d); the "
                         "route is longer than one path may describe\n",
                         where, m_sourceUnit->GetGUIDLow(), int(Nav::Corridor::CAPACITY));
    }
}
...
void PathFinder::BuildPolyPath(const Vector3& startPos, const Vector3& endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        m_route.outcome = m_profile.MayGoDirect(offMeshUnderWater) ? Nav::RouteOutcome::Direct
                                                         : Nav::RouteOutcome::Unroutable;
        m_route.stop = Nav::RouteStop::OffMesh;
        return;
    }
    ...
    if (startPoly == endPoly)
    {
        ...
        m_route.outcome = farFromPoly ? Nav::RouteOutcome::Partial : Nav::RouteOutcome::Routed;
        m_route.stop = farFromPoly ? Nav::RouteStop::Wall : Nav::RouteStop::Reached;
    ...
    const bool startPolyFound = (startIndex != Nav::Corridor::NPOS);
    const bool endPolyFound = (endIndex != Nav::Corridor::NPOS);
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        dtResult = m_navMeshQuery->findPath(
                       ...
                       &suffixPolyLength,
                       int(Nav::Corridor::CAPACITY - prefixPolyLength)); // max polygons out
     ...
     else
    {
        ...
        dtResult = m_navMeshQuery->findPath(
                       ...
                       &polyLength,
                       int(Nav::Corridor::CAPACITY)); // max number of polygons in output path
    ...
    {
        m_route.outcome = Nav::RouteOutcome::Routed;
        m_route.stop = Nav::RouteStop::Reached;
    }
    else
    {
        m_route.outcome = Nav::RouteOutcome::Partial;
        m_route.stop = (m_budgetStop != Nav::RouteStop::Reached) ? m_budgetStop
                                                            : Nav::RouteStop::Wall;
    }
...
{
    float pathPoints[Nav::MAX_POINTS * VERTEX_SIZE];
    uint32 pointCount = 0;
        ...
        (!m_route.IsRouted() || !inRange(getEndPosition(), getActualEndPosition(), 1.0f, 1.0f)))
    {
        ...
        m_route.outcome = Nav::RouteOutcome::Direct;
        m_route.stop = Nav::RouteStop::Forced;
    }
                                    ...
                                    float* smoothPath, int* smoothPathSize, uint32 maxSmoothPathSize)
{
    *smoothPathSize = 0;
    ...
    dtPolyRef polys[Nav::Corridor::CAPACITY];
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        if ((endOfPath || offMeshConnection) && len < Nav::SMOOTH_STEP)
        {
            len = 1.0f;
        }
        else
        {
            len = Nav::SMOOTH_STEP / len;
        }
        ...
        else if (offMeshConnection && inRangeYZX(iterPos, steerPos, Nav::SMOOTH_SLOP, Nav::SMOOTH_HEIGHT))
        {
