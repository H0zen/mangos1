...
// The one exception, and it stays one. A pet follows its owner out over water no
// floor can reach; refusing it the skin there made the route OffMesh, movement fell
// through to a straight line, and the pet swam off across the bay.
//
// Everything else with feet walks the bottom. A makrura is InhabitType GROUND|WATER
// and it does not swim -- carrying the water bit means the water does not stop it.
TEST(MoveProfile_APetRidesTheSkin)
{
    Nav::MoveProfile pet;
    pet.canWalk = true;
    pet.canSwim = true;
    ...
    // Without the exception it is a makrura: feet, so the seabed.
